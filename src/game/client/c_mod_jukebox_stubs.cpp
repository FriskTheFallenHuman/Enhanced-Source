...
static ConCommand cl_stopjukebox( "cl_stopjukebox", StopJukebox, "Stop the jukebox music", FCVAR_DEVELOPMENTONLY );
static ConCommand cl_playrandomtrackfromjukebox( "cl_playrandomtrackfromjukebox", PlayRandomTrackFromJukebox, "Play a random track from the user's jukebox", FCVAR_DEVELOPMENTONLY );
static ConVar cl_jukeboxfadeouttime( "cl_jukeboxfadeouttime", "1.0" );

void CJukeboxPlaylist::AddMusicToPlaylist( const char *szFilename, const char *szHexname, const char *szAlbum, const char *szArtist, const char *szGenre )
{
...
void CJukeboxPlaylist::LoadPlaylistKV()
{
...
void CJukeboxPlaylist::Shutdown()
{
	StopListeningForAllEvents();
}

void CJukeboxPlaylist::FireGameEvent( IGameEvent *event )
{
	if( !event )
		return;
...
void CJukeboxPlaylist::PlayRandomTrack( float fadeInTime /*= 1.0f*/, const char *szDefaultTrack /*= null */ )
{
...
void CJukeboxPlaylist::StopTrack( bool immediate /*= true*/, float fadeOutTime /*= 1.0f */ )
{
	if( m_pCombatMusic )
	{
		if( immediate )
...
void CJukeboxPlaylist::LevelShutdownPostEntity( void )
{
	// Stop music
	StopTrack( true );
	RemoveAllMusic();
...
void CJukeboxPlaylist::ExportPlayistKV( void )
{
...
void CJukeboxPlaylist::SavePlaylist()
{
	Cleanup();
	ExportPlayistKV();
}

const char * CJukeboxPlaylist::GetTrackName( int index )
{
...
const char * CJukeboxPlaylist::GetTrackArtist( int index )
{
	if( index >= m_CombatMusicPlaylist.Count() )
		return null;
	else
...
const char * CJukeboxPlaylist::GetTrackAlbum( int index )
{
	if( index >= m_CombatMusicPlaylist.Count() )
		return null;
	else
...
const char * CJukeboxPlaylist::GetTrackGenre( int index )
{
	if( index >= m_CombatMusicPlaylist.Count() )
		return null;
	else
...
int CJukeboxPlaylist::GetTrackCount()
{
	return m_CombatMusicPlaylist.Count();
}

void CJukeboxPlaylist::RemoveMusicFromPlaylist( const char* szHexnameToRemove )
{
...
void CJukeboxPlaylist::PrepareTrackKV( int index, KeyValues *pKV )
{
...
void CJukeboxPlaylist::MarkTrackForDeletion( int index )
{
	if( index >= m_CombatMusicPlaylist.Count() )
		return;
	else
...
void CJukeboxPlaylist::Cleanup( void )
{
...
void CJukeboxPlaylist::LevelInitPostEntity( void )
{
	RemoveAllMusic();
	// Load the saved playlist
	LoadPlaylistKV();
...
void CJukeboxPlaylist::RemoveAllMusic( void )
{
	StopTrack( true, 0.0f );
	m_CombatMusicPlaylist.RemoveAll();
}

bool CJukeboxPlaylist::IsMusicPlaying( void )
{
